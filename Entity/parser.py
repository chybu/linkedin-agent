...
class Parser:
    ...
    def get_jd(self, job_id:str):
        html = self._fetcher.fetch_job_posting(job_id)
            ...
            jobs.append(
                {
                    ...
                    "company": company,
                    "location": location,
                    "posted": jd["posted"] if "posted" in jd else None,
                    "seniority_level": jd["criteria"]["Seniority level"] if "Seniority level" in jd["criteria"] else None,
                    "employment_type": jd["criteria"]["Employment type"] if "Employment type" in jd["criteria"] else None,
                    "job_function": jd["criteria"]["Job function"] if "Job function" in jd["criteria"] else None,
                    "industry": jd["criteria"]["Industries"] if "Industries" in jd["criteria"] else None,
                    "applicants": jd["applicants"] if "applicants" in jd["criteria"] else None,
                    "description": jd["sections"] if "sections" in jd else None,
                    "source_url": url
                }
            )
